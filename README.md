...
⚠️：push是从本地仓库->远程仓库，而不是工作区，需要先add、commit、再push
⚠️：而pull是从远程仓库->本地、工作区，一次性都更新。
...
当本地分支和远程分支 origin/main 出现了 偏离（diverged），即它们有不同的提交历史，Git 不知道应该用 合并（merge） 还是 变基（rebase） 来同步代码。  

本地修改：你在本地 main 分支提交了代码（git commit），但尚未 git push。
远程修改：与此同时，其他人（或你自己在其他设备）向远程 origin/main 推送了新的提交。
结果：本地仓库和远程分支的提交历史不再一致（即 "偏离"），Git 无法自动决定如何合并你的更改和远程更改。

方法 1：将你的本地提交 变基（rebase） 到远程分支的最新提交上，保持提交历史线性。  
    git pull origin main --rebase  
先将你的本地提交临时移除，拉取远程最新代码，再将你的提交 逐个应用到远程最新提交之上。

方法 2：执行标准的 合并（merge），会创建一个额外的合并提交。
